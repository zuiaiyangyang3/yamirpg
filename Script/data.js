  ...
  saveGlobalData() {
    ...
    switch (Stats.shell) {
      case 'electron': {
        const fs = require('fs')
        const saveDir = File.route('$/Save')
        const targetPath = File.route('$/Save/global.save')
        const backupPath = File.route('$/Save/global.save.bak')
        const cachePath = File.route('$/Save/global.save.tmp')
        // 调试模式下输出格式化的JSON
        // 发布模式下输出压缩后的JSON
    ...
    switch (Stats.shell) {
      case 'electron': {
        ...
        try {
          ...
          try {
            // 如果读取文件或解析JSON失败，则读取备份文件
            const path = File.route('$/Save/global.save.bak')
            const json = await fsp.readFile(path)
            this.globalData = JSON.parse(json)
          } catch (error) {}
        }
